...
function SlopeChart(data, {
    ...
    xPadding = 1.95, // padding for the x-scale (for first and last column)
    ...
    const xAxis = d3.axisTop(xScale).tickSizeOuter(0); //JW change scale
    ...
    const svg = d3.create("svg")
        ...
        .attr("style", "max-width: 100vw; height: auto; height: intrinsic;")
        .attr("font-family", "sans-serif")
        .attr("style", "font-size: 12px;"); // add font size

    svg.append("g")
        .attr("style", "font-size: 16px; ")// add font size
        .attr("transform", `translate(0,${marginTop})`)
        .call(xAxis)
        .call(g => g.select(".domain").remove());
    var colorf = d3.scaleOrdinal(zDomain, ["black", "purple", "lightblue", "green",]);
    svg.append("g")
        .attr("fill", "none")
        .attr("stroke", stroke)
        ...
        .join("path")
        .attr("stroke", ([key]) => { return colorf(key); })
        .style("stroke-dasharray", ([key]) => { console.log(key); return key == "NoVax" ? "10,10" : ""; })
